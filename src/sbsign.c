...
int main(int argc, char **argv)
{
	...
	/*get the msg(data) from the image*/
	get_msg(ctx->image, NULL, &msglen);
	msg = (unsigned char *)malloc(msglen);
	get_msg(ctx->image, msg, &msglen);
    ...
    /*unsigned char to uint_t 8 */
    sig_size = siglen;
    sig_buf = (uint8_t *)malloc(sig_size);
    for(j = 0; j < sig_size; j++)
    {
    	sig_buf[j] = (uint8_t)sig[j];
    }
	

	image_add_signature(ctx->image, sig_buf, sig_size);
